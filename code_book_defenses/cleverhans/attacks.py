...
import code_book_defenses.cleverhans.utils as utils
from code_book_defenses.cleverhans.model import Model, CallableModelWrapper
from code_book_defenses.constants import LOSSES
...
class Attack(object):
    ...
    def __init__(self, model, back='tf', sess=None, loss_type=LOSSES.CE):
        """
...
class SaitoRoyL2(Attack):
    ...
    def __init__(self, model, back='tf', sess=None, loss_type=LOSSES.CE):
        ...
        super(SaitoRoyL2, self).__init__(model, back, sess, loss_type)
    ...
    def generate(self, x, **kwargs):
        ...
        """
        import tensorflow as tf
        from .attacks_tf_v2 import SaitoRoyL2 as SRL2
        self.parse_params(**kwargs)

        labels, nb_classes = self.get_or_guess_labels(x, kwargs)

        attack = SRL2(self.sess, self.model, self.batch_size,
                      self.confidence, 'y_target' in kwargs,
                      self.learning_rate, self.binary_search_steps,
                      self.max_iterations, self.abort_early,
                      self.initial_const, self.clip_min, self.clip_max,
                      nb_classes, x.get_shape().as_list()[1:], centroids=self.centroids)
    ...
    def parse_params(self, centroids=None, y=None, y_target=None, nb_classes=None,
                     batch_size=1, confidence=0,
                     learning_rate=5e-3,
                     binary_search_steps=5, max_iterations=1000,
                     abort_early=True, initial_const=1e-2,
        ...
        self.clip_min = clip_min
        self.clip_max = clip_max
        assert centroids is not None, 'Please pass the centroids'
...
class ElasticNetMethod(Attack):
    ...
    def __init__(self, model, back='tf', sess=None, loss_type=LOSSES.CE):
        ...
        super(ElasticNetMethod, self).__init__(model, back, sess, loss_type)
...
class DeepFool(Attack):
    ...
    def __init__(self, model, back='tf', sess=None, loss_type=LOSSES.CE):
        ...
        super(DeepFool, self).__init__(model, back, sess, loss_type)
...
class LBFGS(Attack):
    ...
    def __init__(self, model, back='tf', sess=None, loss_type=LOSSES.CE):
        ...
        super(LBFGS, self).__init__(model, back, sess, loss_type)
...
class MadryEtAl(Attack):
    ...
    def __init__(self, model, back='tf', sess=None, loss_type=LOSSES.CE):
        ...
        super(MadryEtAl, self).__init__(model, back, sess, loss_type)
        self.feedable_kwargs = {'eps': np.float32,
                                'eps_iter': np.float32,
                                'y': np.float32,
                                'y_target': np.float32,
    ...
    def attack_single_step(self, x, eta, y):
        ...
        """
        import tensorflow as tf
        from code_book_defenses.cleverhans.utils_tf import model_loss, clip_eta
    ...
    def attack(self, x, y):
        ...
        """
        import tensorflow as tf
        from code_book_defenses.cleverhans.utils_tf import clip_eta
...
class FastFeatureAdversaries(Attack):
    ...
    def __init__(self, model, back='tf', sess=None, loss_type=LOSSES.CE):
        ...
        """
        super(FastFeatureAdversaries, self).__init__(model, back, sess, loss_type)
        self.feedable_kwargs = {'eps': np.float32,
                                'eps_iter': np.float32,
                                'clip_min': np.float32,
                                'clip_max': np.float32,
