...
from code_book_defenses.constants import LOSSES, ACTIVATIONS
...
def model_loss(y, model, loss_type, mean=True):
    ...
    op = model.op
    if op.type == ACTIVATIONS.SOFTMAX:
        logits, = op.inputs
    else:
        logits = model

    if loss_type == LOSSES.CE:
        out = tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=y)
    elif loss_type == LOSSES.MSE:
...
def model_train(sess, x, y, predictions, X_train, Y_train, save=False,
                ...
                feed=None, args=None, rng=None, loss_type=LOSSES.CE):
    """
    Train a TF graph
